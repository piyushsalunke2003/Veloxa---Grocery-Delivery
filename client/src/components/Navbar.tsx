import { ArrowUpRightIcon, BikeIcon, ChevronDown, LogOutIcon, MenuIcon, SearchIcon, ShieldIcon, ShoppingCartIcon, UserIcon, XIcon } from 'lucide-react';
import { useState } from 'react'
import { Link, useNavigate } from 'react-router-dom'

const Navbar = () => {
    const user: any = {name: "John Doe", email: "john.doe@example.com", isAdmin : true}
                                ...
                                <>
                                    ...
                                    <div className='absolute right-0 mt-2.5 w-56 bg-white rounded-xl shadow-lg border border-app-border py-2 z-50 animate-fade-in'>
                                        ...
                                        <div onClick={ ()=> setUserMenuOpen(false)}>
                                            ...
                                            {user && <Link to='/orders' className='dropdown-link'><UserIcon size={16} /> My Orders </Link>}
                                            {user && <Link to='/addresses' className='dropdown-link'><UserIcon size={16} /> Addresses </Link>}
